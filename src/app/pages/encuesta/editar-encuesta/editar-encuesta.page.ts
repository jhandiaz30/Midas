...
export class EditarEncuestaPage implements OnInit {
  ...
  //METODO PARA CARGAR EL ADMINISTRADOR DEL CONTRATO, NUMERO DE CONTRATO Y EMPRESA DE ACUERDO AL SELECT
  ...
  public async editarRtasEncuesta() {
    ...
    for (var i = 0; i < this.loadPregunta.length; i++) {
      ...
      if (obs != '' && this.Form.obs[i] != undefined && this.loadOpc[i] == undefined) {
        aux2 = aux2 + 1;
  ...
  dataPDF() {
    ...
    for (let index = 0; index < this.loadPregunta.length; index++) {
      const element = {
        ...
        respuesta: this.opc[index],
        observacion: this.Form.obs[index],
